...
func testDiffAccounts(t *testing.T, client *rpc.Client) {
	...
	for _, testBlock := range testBlocks {
		...
		diffDetail, err := ec.GetDiffAccountsInBlock(ctx, big.NewInt(int64(testBlock.blockNr)), accounts)
		if err != nil {
			t.Fatalf("get diff accounts in block error: %v", err)
		}
		// No contract deposit tx, so all accounts expect 0 value.
		for _, diff := range diffDetail.Transactions {
			for _, account := range accounts {
				if diff.Accounts[account].Cmp(big.NewInt(0)) != 0 {
					t.Fatalf("account(%v) value expected 0, but get %v", account, diff.Accounts[account])
				}
			}
		}
	}
